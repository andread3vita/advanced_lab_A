...
void InfoAcq::FillSettings(chSettings *ch1, chSettings *ch2,
                           samplingSettings *samp) {
  if (ch1->enabled) {
...
void InfoAcq::PrintInfo() {
  ...
  for (int32_t i = 0; i < 2; i++) {
    ...
    if (!chSet[i].enabled)
      ...
      cout << "ENABLED\n";
      cout << "Coupling\t\t";
      if (chSet[i].DCcoupled == 'D')
        cout << "DC\n";
      else if (chSet[i].DCcoupled == 'A')
        cout << "AC\n";
      cout << "Range (mV)\t\t" << inputRanges[chSet[i].range] << endl;
      cout << "Offset (mV)\t\t" << chSet[i].analogueOffset << endl;
      if (!chSet[i].triggerSet)
        cout << "TRIGGER NOT SET\n";
      else if (chSet[i].triggerSet) {
        cout << "TRIGGER SET\n";
        cout << "Trigger direction\t";
        if (chSet[i].triggerDirection == 'R')
          cout << "RISING\n";
        else if (chSet[i].triggerDirection == 'F')
          cout << "FALLING";
  ...
  cout << "Number of samples per waveform\t\t" << sampSet.samplesStoredPerEvent
       << endl;
  cout << "Number of pretrigger samples\t\t" << sampSet.preTrig << endl;
  cout << "Sampling interval (ns)\t\t" << sampSet.timeIntervalNanoseconds
       << endl;
  cout << "\n";
}

void InfoAcq::CreateChannelBranches(TTree *treeCh) {
  treeCh->Branch("Ch1", &chSet[0].enabled,
                 "Enabled/O:Coupling/C:Range/S:Offset/F:TriggerSet/O:Direction/"
                 "C:Threshold/F");
  treeCh->Branch("Ch2", &chSet[1].enabled,
                 "Enabled/O:Coupling/C:Range/S:Offset/F:TriggerSet/O:Direction/"
                 "C:Threshold/F");
}
...
void InfoAcq::CreateSamplingBranches(TTree *treeSamp) {
  ...
  treeSamp->Branch(
      "Settings", &sampSet.max_adc_value,
      "MaxAdcValue/I:nSegments/i:nWaveforms/i:Timebase/i:nSamplesDIV/"
      "I:SamplesPerWaveform/I:PreTrig/I:DeltaT_ns/F");
}
