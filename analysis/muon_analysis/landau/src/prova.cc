...
//				          ***New ***
/*
double weighted_mean_with_error(double x[], double sigma[],int n, double
&sigma_mean){
...
cerr<<" \n \n --->Average : "<<mean<<" +- "<<sigma_mean<<'\t'<<(sigma_mean/mean
*100.)<<" % "<<endl;
...
//	int findMaximumSiPM(string filename, float& offset, float& eOffset,
// float& gain, float& eGain)
...
int findMaximumSiPM(string filename1, string filename2, string filename3 /* ,int nsigma*/, int nPeaks = 4, int ch = chB,
                    int polarity = negative, Long64_t nEvt = -1)
{
    ...
    */
    ...
    // file << f->GetParameter(0) << " " << f->GetParError(0) << " " <<
    // f->GetParameter(1) << " " << f->GetParError(1) << endl;
    ...
    for (int i = 1; i < nPeaks; i++)
    {
        ...
        /////////////////////////////
                ...
                areas[i-1] = hMax->Integral(binMin,binMax);
                cerr<< "   # Area (from  "<<binMin<< " to "<<binMax<<" bins) :
        "<<areas[i-1]<<"\n"<<endl;
        */
    }
    ...
    /*
            ...
            float tmp_sigma_mu = (1./areas[i-2])* TMath::Sqrt( areas[i-1]* ( 1.+
       (areas[i-1]/areas[i-2])  )	); mu_s[i-2] = tmp_mu; sigma_mu_s[i-2] =
       tmp_sigma_mu; cerr<< " /mu_{"<<(i-1)<<"} : "<< tmp_mu<<" +-
       "<<tmp_sigma_mu<<endl;
            }
            double sigma_mu_mean ;
            double mu_mean =
       weighted_mean_with_error(mu_s,sigma_mu_s,nPeaks-2,sigma_mu_mean);
    */
