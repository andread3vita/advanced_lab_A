...
Double_t chi_sqr(Double_t *val, Double_t *par)
{
    ...
    Where f(t) plays the role of the expected value and binvalue(t) is the hight of the bin with center t measured
    during the experiment. The function is a 2Dim function of (x,y) = (normalization,life time).
    */
...
TF1 *FitData(std::string filename, int xmin_fit = 2000, int xmax_fit = 16000, int total_bins = 75)
{
    ...
    */
    ...
    while (input >> value)
    {
        h->Fill(calibration(value, std::stod(calibration_parameters->ValueOf("m")),
                            std::stod(calibration_parameters->ValueOf("q"))));
    }
    ...
    c_chi->SetGrid();
    c_chi->SetLogz();
    TF2 *f_chi = new TF2("f_chi", chi_sqr, f->GetParameter(1) - 40 * f->GetParError(1),
                         f->GetParameter(1) + 40 * f->GetParError(1), f->GetParameter(2) - 40 * f->GetParError(2),
                         f->GetParameter(2) + 40 * f->GetParError(2), 1);
    ...
    f_chi->Draw("CONT4Z");
    c_chi->SaveAs("./correlation_chi_sqr.png"); // I can't export a pdf since from there it can be seed that the plot is
                                                // made point by point. If I use png the resolution is not enought and
                                                // the overall plot looks smoother.
...
void Fit_various_intervalas(int min_left, int max_left, int step, int max_right)
{
    ...
    for (int i = min_left; i <= max_left; i = i + step)
    {
        ...
        out << i << "-" << max_right << ":\t" << res_tmp->GetParameter(2) << "\t" << res_tmp->GetParError(2) << "\t"
            << res_tmp->GetChisquare() / res_tmp->GetNDF() << std::endl;
    }
    return;
}
