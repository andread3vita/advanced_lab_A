...
void ReadTree(const char* fileName, int i, bool negative = false,
              bool draw = true, int channel = chB) {
  // dichiaro le struct
  ...
  */
  ...
  // spettro in energia
  TH1F* spectrumIntegral =
      new TH1F("hInt", "Integral Spectrum", 200, negative ? -5e4 : -1e3,
               negative ? 1e3 : 5e4);
  TH1F* spectrumIntegralMissed = new TH1F(
      "hIntMiss", "Integral Spectrum of missed events", 200, -5e4, 5e4);
  float xmin = negative ? adc_to_mv(sampSet.max_adc_value, chSet.range,
                                    -1 * sampSet.max_adc_value)
                        : 0;
  float xmax = negative ? 0
                        : adc_to_mv(sampSet.max_adc_value, chSet.range,
                                    sampSet.max_adc_value);
  TH1F* spectrumMaximum =
      new TH1F("hMax", "Maximum Spectrum", 1024, xmin, xmax);
  TH1F* spectrumMaximumMissed = new TH1F(
      "hMaxMiss", "Maximum Spectrum of missed events", 180,
      -adc_to_mv(sampSet.max_adc_value, chSet.range, sampSet.max_adc_value),
      adc_to_mv(sampSet.max_adc_value, chSet.range, sampSet.max_adc_value));
  TH1F* spectrumMaximumSig = new TH1F(
      "hMaxSig", "Maximum Spectrum of signal events", 180,
      -adc_to_mv(sampSet.max_adc_value, chSet.range, sampSet.max_adc_value),
      adc_to_mv(sampSet.max_adc_value, chSet.range, sampSet.max_adc_value));
  for (Long64_t index = 0; index < nEvt; index++) {
    ...
    for (int ii = 0; ii < sampSet.samplesStoredPerEvent; ii++) {
      float value = adc_to_mv(sample[ii], chSet.range, sampSet.max_adc_value);
      // printf ("valure :%f\n",value);
      integral += value;
  ...
  cerr << "DEBUG!!!!!: " << daVedere << " E QUESTO: " << waveformInBlock
       << endl;
...
void viewEvent(TFile* input_file, int i, int channel = chB) {
  // dichiaro le struct
  InfoAcq::chSettings       chSet1;
  InfoAcq::chSettings       chSet2;
  InfoAcq::samplingSettings sampSet;
